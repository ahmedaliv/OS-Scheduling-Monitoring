...
int main()
{
    ...
    while (fgets(line, MAX_LINE_LENGTH, fp) != NULL) {
        ...
        sscanf(line, "%d | %s | %d | %d | %d | %d | %s", &processes[i].process_id, processes[i].process_name, &processes[i].arrival_time, &processes[i].CPU_time, &processes[i].io_time, &processes[i].io_completion_time, processes[i].pro_specifier);
        // Increment the index for the processes array
        processes[i].burst_time = processes[i].CPU_time - processes[i].io_time;
        i++;
        num_ofDataset_processes++;
    }
    num_processes= num_ofDataset_processes;
    ...
    scanf("%d",&choose);
    if(choose==1)mlfq(processes,num_ofDataset_processes);
